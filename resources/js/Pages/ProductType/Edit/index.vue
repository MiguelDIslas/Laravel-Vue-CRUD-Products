<template>
    <div>
        <app-layout title="Edit product type">
            <h1
            ...
            >
               Edit product type: {{ this.$props.productType.name }}
            </h1>
            <div class="py-12">
                        ...
                        <form @submit.prevent="submit" class="mb-6">
                            ...
                            >
                                ...
                                <div class="grid grid-cols-1">
                                    ...
                                    <select
                                        class="form-select appearance-none block w-full px-3 py-1.5 text-base font-normal text-gray-700 bg-white bg-clip-padding bg-no-repeat border border-solid border-gray-300 rounded transition ease-in-out m-0 focus:text-gray-700 focus:bg-white focus:border-blue-600 focus:outline-none"
                                        aria-label="Select category" v-model="form.category_id"
                                    >
                                        <option disabled>
...
export default {
    props: ['productType', 'categories'],
    name: "ProductTypeFormEdit",
    components: {
        AppLayout,
        Link,
    },
    data() {
        return {
            form: {
                category_id: this.$props.productType.category_id,
                name: this.$props.productType.name,
                description: this.$props.productType.description,
            },
        };
    },
    methods: {
        submit() {
            this.$inertia.put(route("productTypes.update", this.$props.productType.id), this.form);
        },
    },
};
</script>
